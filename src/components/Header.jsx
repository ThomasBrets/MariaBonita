...
const Header = () => {
  ...
  return (
    <header
      className={`${
        header ? "bg-primary py-6 shadow-lg" : "bg-transparent py-8"
      } fixed z-50 w-full transition-all duration-500`}
    >
      <div className="container mx-auto  gap-y-6 flex flex-col items-center lg:flex-row lg:justify-between lg:gap-y-0">
        <a href="/">
          <img src={logo} alt="logo" className="w-[140px] h-[140px] rounded-full p-2" />
        </a>
        <nav className={`${header ? "text-secondary" : "text-black"} font-primary flex gap-x-4 uppercase text-[18px] tracking-3px items-center lg:gap-x-8`}>
